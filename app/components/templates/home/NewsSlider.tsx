            ...
            {props?.map((item, index) => (
              <SwiperSlide key={index}>
                <div className="flex max-lg:flex-col items-center max-lg:gap-3 gap-12 justify-between">
                  <Image
                    className={`max-sm:h-auto max-lg:rounded-md w-[300px] object-cover h-[270px] max-sm:object-cover transition-transform ${index === activeIndex ? "max-lg:translate-y-0" : "max-lg:translate-y-7"}`}
                    height={400}
                    width={400}
