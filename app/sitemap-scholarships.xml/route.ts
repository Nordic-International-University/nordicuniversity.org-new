  ...
  try {
    ...
    for (const lang of LOCALES) {
      const res = await fetch(
        `${backendUrl}/api/cooperation/meeting/admin?type=SCHOLARSHIPS_AND_INTERNSHIPS&page=1&limit=100&language=${lang}`,
        {
    ...
    const urls = allItems.flatMap((item) =>
      LOCALES.map((locale) => ({
        loc: `${BASE_URL}/${locale}/partners/scholarships-and-internships/${item[`slug_${locale}`]}`,
