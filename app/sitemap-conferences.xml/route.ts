  ...
  try {
    ...
    for (const lang of LOCALES) {
      const res = await fetch(
        `${backendUrl}/api/science/meeting/admin?type=CONFERENCES&page=1&limit=100&language=${lang}`,
        {
    ...
    const urls = allItems.flatMap((item) =>
      LOCALES.map((locale) => ({
        loc: `${BASE_URL}/${locale}/research/scientific-conferences/${item[`slug_${locale}`]}`,
    ...
    console.error("Error generating conferences sitemap:", error);
